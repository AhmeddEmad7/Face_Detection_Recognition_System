...
function FaceRecognitionPage() {
  ...
  const onApplyFaceRecognition = async () => {
    if (!imageFile) return;
    const data = await ImageOpsManager.applyFaceRecognition(imageFile);
    ...
    <div className="h-full flex-1 flex-col space-y-8 p-8 flex">
      <div className="flex items-center justify-between space-y-2">
        <div>
          ...
          <p className="text-muted-foreground">
            Recongize a face in an image from our ORL dataset using AI and Computer Vision
          </p>
        </div>
        {imageFile && (
          <div className="flex flex-col gap-2 items-center justify-center">
            <Button onClick={onApplyFaceRecognition} className="w-[160px]">
              Apply Recognition
            </Button>
            <Button
      ...
      {imageFile && (
        <div className="grid grid-cols-1 md:grid-cols-2 items-center justify-around gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Label>Face Recognition Result</Label>
            <div className="border rounded-md">
              <img
                src={resultImage}
                alt="placeholder"
